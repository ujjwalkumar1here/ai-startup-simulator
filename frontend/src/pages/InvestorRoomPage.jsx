...
export default function InvestorRoomPage() {
  ...
  useEffect(() => {
    ...
    const resumeSession = async () => {
      ...
      setResumeError(null);
      try {
        const res = await investorService.getSession(resumeSessionId);
        if (!active) return;
        const existingSession = res.data;
        const existingAnswers = existingSession.answers || [];
        setSessionId(existingSession._id);
        setQuestions(existingSession.questions || []);
        setCurrentQ(existingAnswers.length);
        setFeedbacks(existingAnswers.map((entry) => ({ ...entry, userAnswer: entry.answer })));
        setFinalReport(existingSession.feedback || null);
        setSessionCompleted(existingSession.status === "completed");
        setAnswer("");
      } catch (e) {
        if (active) setResumeError(!e?.response ? "network" : e.response.status === 401 ? "unauthorized" : "default");
      } finally {
        if (active) setResumingSession(false);
      }
    };

    resumeSession();
    return () => { active = false; };
  }, [resumeSessionId, resumeAttempt]);
  ...
  /* ─── loading / error ────────────────────────── */
  if (loadingSims || resumingSession) {
    return (
  ...
  if (simsError || resumeError) {
    return <ErrorState type={simsError || resumeError} onRetry={resumeError ? () => setResumeAttempt((attempt) => attempt + 1) : fetchSimulations} />;
  }
        ...
        <SectionCard icon={MessageSquareText} title="Full Q&A Transcript">
          <div className="space-y-4">
            {feedbacks.map((fb, idx) => {
              ...
              return (
                <div key={idx} className="rounded-xl border border-slate-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                  ...
                  <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{q.question}</p>
                  <p className="mt-2 rounded-lg bg-slate-100 p-3 text-sm text-slate-800 dark:bg-slate-800/60 dark:text-slate-200">
                    <span className="font-semibold text-slate-900 dark:text-slate-100">Your answer:</span> {fb.userAnswer}
                  </p>
                  <p className="mt-2 text-sm italic text-slate-600 dark:text-slate-400">"{fb.investorComment}"</p>
                </div>
              );
            })}
          </div>
